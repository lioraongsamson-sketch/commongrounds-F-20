...
class BookListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            contributions = context["contributed_books"] = all_books.filter(
                contributor=profile
            )
            bookmarks = context["bookmarked_books"] = all_books.filter(
                bookmark__profile=profile
            )
            reviews = context["reviewed_books"] = all_books.filter(
...
class BookDetailView(DetailView):
    ...
    ctx = {"bookreview_list": reviews, "reviewgroup": reviewgroups}
        ...
        if self.request.user.is_authenticated:
            if request.POST.get("to_bookmark") == "bookmark":
                bookmarked_book = Bookmark.objects.filter(
                    book=book, profile=self.request.user.profile
                )
